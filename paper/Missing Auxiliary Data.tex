...
\section{Design-Based Estimators When \texorpdfstring{$\mathbf{x}$}{x} is Missing on the Frame}
...
\subsection{Stratification by \texorpdfstring{$\mathbf{x}$}{x}-Observability (Domain Estimation)}
...
\subsection{IPW for \texorpdfstring{$\mathbf{x}$}{x}-Observability on the Frame}
...
\subsection{AIPW (Doubly Robust) for \texorpdfstring{$f(\mathbf{x})$}{f(x)}}
