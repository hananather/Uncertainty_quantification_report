...
\]
This estimator combines the best of both worlds. The first term $\sum_U \hat{m}_S(\mathbf{z}_i)$ leverages the frame data $\mathbf{z}$ to reduce variance, and also ensures the estimate is calibrated to known population totals. The second term acts as a bias correction. Crucially because $y_i^\star$ contains the information from $\mathbf{x}_i$, this effectively ``imports" the predictive power of $\mathbf{x}$ into the population estimate without requiring $\mathbf{x}$ to be known for the whole population.
...
\]
This estimator is unbiased only if the change in outcome $y_i - y_i^{(t-1)}$ is uncorrelated with the response probability, or if the change is constant across the population. It fails to utilize $\mathbf{x}_i$ to account for heterogeneous evolution of the variable of interest.
...
\[
\hat{\mu}_{\text{Pre-train}} = \frac{1}{N} \left( \sum_{i \in U} f_{\text{hist}}(\mathbf{z}_i) + \sum_{i \in S_r} \frac{y_i - f_{\text{hist}}(\mathbf{z}_i)}{\pi_i} \right)
\]
...
Recent literature in survey sampling emphasizes that while Double Robustness protects against the misspecification of one model, the finite-sample performance of the estimator depends heavily on the stability and predictive power of these models \cite{Haziza2017}. Consequently, we adopt a hybrid modeling strategy that leverages the strengths of machine learning for prediction and the stability of parametric estimation for weighting.

\subsection{Imputation Model: Gradient Boosted Trees (XGBoost)}

For the imputation step, our objective is to minimize the mean squared error of the predictions $\tilde{y}_i$. The relationship between the historical administrative data ($\mathbf{z}_i$) and the current sample data ($\mathbf{x}_i$) is may be complex and non-linear. We use \textbf{XGBoost} (eXtreme Gradient Boosting), a scalable implementation of gradient boosted decision trees \cite{Chen2016}. XGBoost constructs an ensemble of weak learners (regression trees) that sequentially correct the residuals of prior trees. We select XGBoost for this simulation over other methods for three specific reasons:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Refit the projection model $\hat{m_S}^{(b)}(\mathbf{z}_i)$ on the completed bootstrap sample.
    \end{itemize}
