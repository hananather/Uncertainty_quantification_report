...
\begin{description}
   ...
   \end{cases}
    ...
    \]
    We denote the response propensity as $p_i(\mathbf{v}_i) := \Pr(r_i=1\mid \mathbf{v}_i)$.
...
% ------------------ SCENARIO 2 ------------------- %
% ------------------------------------------------- %
\paragraph{Scenario 2A: Item Nonresponse with Population Auxiliary Data.}
...
\hat{\mu}_{\text{NR-GREG}}
  = \frac{1}{N}\left(
      \sum_{i\in U} \hat{m}_{S_{r}}(\mathbf{z}_i)
            + \sum_{i\in S_r}
          \frac{1}{\hat{p}_i \pi_i} (y_i - \hat{m}_{S_r}(\mathbf{z}_i))
    \right),
\end{equation}
...
\paragraph{Scenario 2B: GREG$_{\hat y}$ with a Pre-trained Historical Prediction.}

Assume a pre-trained (historical) prediction function $f_{\text{hist}}$ is available and
produces a scalar prediction for every unit in the population:
\[
\hat y_i^{\text{hist}} := f_{\text{hist}}(\mathbf z_i), \qquad i\in U.
\]
Define the (low-dimensional) calibration vector
\[
\mathbf u_i := \begin{bmatrix} 1 \\ \hat y_i^{\text{hist}} \end{bmatrix}.
\]
Let $\hat m(\hat y) := \hat\beta_0 + \hat\beta_1 \hat y$ denote the linear working model
for regressing the current outcome on the historical prediction.

\medskip
\noindent\textbf{Complete-response version (no item nonresponse).}
If $y_i$ is observed for all $i\in S$, the GREG$_{\hat y}$ estimator is
\begin{equation}\label{eq:s2_greg_yhat_complete}
\hat\mu_{\mathrm{GREG}_{\hat y}}
=
\frac{1}{N}\left(
\sum_{i\in U} \hat m(\hat y_i^{\text{hist}})
+
\sum_{i\in S}\frac{1}{\pi_i}\Bigl(y_i-\hat m(\hat y_i^{\text{hist}})\Bigr)
\right),
\end{equation}
where $\hat m(\cdot)$ is fit on the sample via weighted least squares of $y_i$ on
$\mathbf u_i$ using design weights $d_i=1/\pi_i$.

\medskip
\noindent\textbf{Item-nonresponse version (NR-GREG$_{\hat y}$).}
When $y_i$ is observed only for respondents ($r_i=1$), we use the standard
nonresponse GREG form with a response propensity model $\hat p_i$:
\begin{equation}\label{eq:s2_greg_yhat_nr}
\hat\mu_{\mathrm{NR\text{-}GREG}_{\hat y}}
=
\frac{1}{N}\left(
\sum_{i\in U} \hat m(\hat y_i^{\text{hist}})
+ \sum_{i\in S_r} \frac{1}{\pi_i \hat p_i}\Bigl(y_i-\hat m(\hat y_i^{\text{hist}})\Bigr)
\right).
\end{equation}
Here $\hat p_i \approx \Pr(r_i=1\mid \mathbf v_i)$ can depend on the full sample
auxiliaries $\mathbf v_i=[\mathbf z_i,\mathbf x_i]$, while the outcome regression
$\hat m(\hat y)$ uses only $\hat y_i^{\text{hist}}$ so that the population total
$\sum_{i\in U}\hat m(\hat y_i^{\text{hist}})$ is computable.

\medskip
\noindent\textbf{Connection to difference estimation (PPD / historical anchor).}
Equation~\eqref{eq:s2_greg_yhat_complete} reduces to the prediction-powered difference
estimator when $\hat m(\hat y)=\hat y$ (i.e., $\hat\beta_0=0,\ \hat\beta_1=1$).
Thus GREG$_{\hat y}$ can be interpreted as a \emph{linear re-calibration} of the
historical prediction that corrects for simple drift via an intercept and slope.
...
= \frac{1}{N} \left(
  \sum_{i \in U} f_{\text{hist}}(\mathbf{z}_i)
   + \sum_{i \in S_r} \frac{y_i - f_{\text{hist}}(\mathbf{z}_i)}{\pi_i}
\right)
\end{equation}
...
\hat{\mu}_{\text{Hist-PPD-DR}} = \frac{1}{N} \left( 
    ...
    \;+\; 
        \sum_{i\in S_r} \frac{1}{\pi_i \hat{p}_i}\,\Bigl(y_i - \tilde{y}_i\Bigr)
\right)
\end{equation}
...
% ------------------------------------------------- %
\section{Model Calibration with Historical Priors}
\label{sec:model_calibration_framework}

We incorporate historical outcomes $y_i^{(t-1)}$ through a prediction function
$f_{\text{hist}}(\mathbf{z}_i)$ trained on administrative/historical data. Rather than adding
these historical predictions and all frame variables $\mathbf{z}_i$ directly as
covariates in a large regression, which can lead to unstable weights and
multicollinearity, we follow the model-calibration approach of
\textcite{wu_sitter_2001_jasa}. In this framework, $f_{\text{hist}}(\mathbf{z}_i)$
is used as a single calibration / projection variable that summarizes the
historical information.


\subsection{Dimensionality Reduction and Stability}

The core idea is to replace a high-dimensional auxiliary vector $\mathbf{z}_i$
with a parsimonious prediction scalar:
\[
  \hat{y}_i^{\text{hist}} = f_{\text{hist}}(\mathbf{z}_i).
\]
Calibrating on this scalar (or using it as the projection term in a difference
estimator) reduces the dimensionality of the problem from $p_z$ frame variables
to a single summary. This tends to produce better-conditioned, more stable
weights, even when $f_{\text{hist}}$ is complex (e.g., a non-linear machine
learning model) or when the components of $\mathbf{z}_i$ are highly correlated
\parencite{beaumont_bocci_2008_penalized}.

\subsection{Decoupling Prediction from Correction}

The \textbf{Hist-PPD} estimator (Scenario~4) illustrates this structure. It
separates the \emph{historical anchor} from the \emph{current correction}:
\begin{itemize}
  \item \textbf{Anchor (variance reduction):} use the population sum
        $\sum_{i \in U} f_{\text{hist}}(\mathbf{z}_i)$, which has zero sampling
        variance because it is evaluated on the frame.
  \item \textbf{Correction (bias reduction):} use the current sample to estimate
        the residual $(y_i - f_{\text{hist}}(\mathbf{z}_i))$.
\end{itemize}
The same anchor–correction decomposition underlies both the simple
GREG$_{\hat y}$ estimator in Scenario~2B and the Hist-PPD / Hist-PPD-DR
estimators in Scenarios~4–5: historical predictions stabilize the projection
term, while current auxiliaries $\mathbf{x}_i$ are used only to correct for
concept drift and nonresponse.


\subsection{Valid Inference with Generated Regressors}

In our setting the historical quantity
\[
  \hat{y}_i^{\text{hist}} = f_{\text{hist}}(\mathbf{z}_i)
\]
is not an ordinary auxiliary variable: it is constructed by applying a prediction model
$f_{\text{hist}}$ fitted on historical data. In the terminology of \textcite{pagan_1984_econometrics},
$\hat{y}_i^{\text{hist}}$ is a \emph{generated regressor}.

Using $\hat{y}_i^{\text{hist}}$ as the calibration / projection variable, rather than as just another
covariate, keeps the design-based structure of the estimator transparent. The Hist-PPD and
Hist-PPD-DR estimators can both be written as a population-level ``anchor'' based on
$f_{\text{hist}}(\mathbf{z}_i)$ plus a sample-based correction term involving residuals.

The replication scheme in Section~\ref{sec:variance_estimation} then provides a natural way
to obtain standard errors: each bootstrap replicate recomputes the estimator using the same
procedure, so the resulting variance estimator reflects the additional variability induced by
working with the generated regressor $\hat{y}_i^{\text{hist}}$. In our experiments this leads
to confidence intervals with close-to-nominal coverage \parencite{cbs_2025_measurement}.
...
Therefore, our simulation utilizes XGBoost for $m_S(\cdot)$ to maximize predictive accuracy, and Logistic Regression for $p(\cdot)$ to ensure stability and calibration of the weights.
...
Standard design based variance formulas treat completed outcomes (for example, mass imputed values)
as if they were observed. This ignores the uncertainty from fitting the imputation model and can
produce standard errors that are too small. We therefore use a replication approach.

\paragraph{Design based bootstrap.}
Let $S$ denote the realized probability sample with design weights $d_i = 1/\pi_i$.
For $b=1,\dots,B$ we draw a bootstrap sample $S^{(b)}$ from $S$ using a design based bootstrap
appropriate for the sampling design. In each bootstrap replicate we refit any models that would
be refit in practice (imputation models, projection models, propensity models) and recompute the
estimator. The bootstrap variance of an estimator $\hat\mu$ is
\[
\widehat{\mathrm{Var}}(\hat\mu)
=
\frac{1}{B-1}\sum_{b=1}^B\left(\hat\mu^{(b)}-\bar\mu_B\right)^2,
\qquad
...
We conduct Monte Carlo simulations under a finite population design to evaluate the estimators
defined in Scenarios~1 to~5. In each experiment we generate a finite population $U$ of size $N$,
draw a probability sample $S$ of size $n$, induce item nonresponse in $y_i$, compute point estimates,
and construct confidence intervals using the bootstrap variance estimator in
Section~\ref{sec:variance_estimation}. The target parameter is the finite population mean
\[
\mu = \frac{1}{N}\sum_{i\in U} y_i.
\]

\subsection{Benchmark data generating models}
\label{subsec:sim_dgp}

Each benchmark specifies an underlying covariate vector $\mathbf{w}_i$ and an outcome
\[
y_i = g(\mathbf{w}_i) + \varepsilon_i,
\]
with independent errors. We use the following benchmark functions.

\paragraph{Multi linear (5 covariates).}
Draw $w_{ij}\sim \mathcal{U}(0,1)$ for $j=1,\dots,5$ and define
\[
y_i = \sum_{j=1}^5 w_{ij} + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,0.1^2).
\]

\paragraph{Friedman1 (5 covariates).}
Draw $w_{ij}\sim \mathcal{U}(0,1)$ for $j=1,\dots,5$ and define
\[
y_i = 10\sin(\pi w_{i1}w_{i2}) + 20(w_{i3}-0.5)^2 + 10w_{i4} + 5w_{i5} + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,0.1^2).
\]

\paragraph{Friedman2 (4 covariates).}
Draw
\[
w_{i1}\sim \mathcal{U}(0,100),\quad
w_{i2}\sim \mathcal{U}(40\pi,560\pi),\quad
w_{i3}\sim \mathcal{U}(0,1),\quad
w_{i4}\sim \mathcal{U}(1,11),
\]
and define
\[
y_i = \sqrt{w_{i1}^2 + \left(w_{i2}w_{i3} - \frac{1}{w_{i2}w_{i4}}\right)^2} + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,0.1^2).
\]

\paragraph{Friedman3 (4 covariates).}
Using the same supports as Friedman2, define
\[
y_i = \arctan\!\left(\frac{w_{i2}w_{i3} - \frac{1}{w_{i2}w_{i4}}}{w_{i1}}\right) + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,0.1^2).
\]

\paragraph{Haziza1 (4 covariates).}
Draw $w_{ij}\sim \mathcal{U}(-100,100)$ for $j=1,\dots,4$ and define
\[
y_i = 2 + w_{i1}^2 + w_{i2}^2 + w_{i3}^3 + w_{i4} + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,1).
\]

\paragraph{Haziza2 (4 covariates).}
Draw $w_{ij}\sim \mathcal{U}(-100,100)$ for $j=1,\dots,4$ and define
\[
y_i = 2 + 2w_{i1} + 5w_{i2} + w_{i1}^2 w_{i3}^2 w_{i4}^2 + \varepsilon_i,
\qquad \varepsilon_i\sim \mathcal{N}(0,1).
\]

\subsection{Frame variables, sample only variables, and drift}
\label{subsec:sim_covariates}

To reflect the distinction used throughout the paper, we split $\mathbf{w}_i$ into
frame variables $\mathbf{z}_i$ and sample only variables $\mathbf{x}_i$ as follows.

For the 5 covariate benchmarks, define
\[
\mathbf{z}_i = (w_{i1}, w_{i2})^\top,\qquad \mathbf{x}_i = (w_{i3}, w_{i4}, w_{i5})^\top.
\]
For the 4 covariate benchmarks, define
\[
\mathbf{z}_i = (w_{i1}, w_{i2})^\top,\qquad \mathbf{x}_i = (w_{i3}, w_{i4})^\top.
\]
In the simulation we generate the full $\mathbf{w}_i$ for every unit in $U$, but the estimators are
restricted to the same information pattern as in the paper: $\mathbf{z}_i$ is treated as known on
the full population and $\mathbf{x}_i$ is treated as observed only for sampled units.

To create a simple and controlled form of drift, we use different distributions for the sample only
components across the historical and current periods. Specifically, in the historical data we draw
all covariates according to the benchmark distributions above. In the current population we draw
the frame components using the same benchmark distributions, but we draw each sample only component
by transforming a $\mathrm{Beta}(2,5)$ variable onto the benchmark support. This changes the
distribution of $\mathbf{x}_i$ across periods while holding the support fixed.

\subsection{Finite population and sampling}
\label{subsec:sim_sampling}

For each benchmark model we generate one finite population $U$ of size $N=20{,}000$ and compute the
true finite population mean $\mu$. We then run sampling experiments for sample sizes
\[
n \in \{50, 100,\ 500,\ 2{,}000\},
\]
and for each $n$ we repeat the experiment for $K$ Monte Carlo replicates. We use simple random
sampling without replacement. Under this design, $\pi_i = n/N$ and $d_i = N/n$.

\subsection{Item nonresponse mechanism}
\label{subsec:sim_nonresponse}

Within each realized sample $S$ we generate the item response indicator $r_i$ using a logistic
propensity model that depends on sample only variables. Let $x_{i1}$ denote the first coordinate of
$\mathbf{x}_i$ and define a standardized index
\[
u_i = \frac{x_{i1} - \bar{x}_{1,S}}{s_{1,S}},
\]
where $\bar{x}_{1,S}$ and $s_{1,S}$ are the sample mean and sample standard deviation of $x_{i1}$
over the realized sample $S$. We then set
\[
\Pr(r_i=1 \mid \mathbf{z}_i,\mathbf{x}_i) = p_i,
\qquad
\mathrm{logit}(p_i) = \eta_0 + \eta_1 u_i,
\]
with $\eta_1 = 0.7$ and $\eta_0 = \mathrm{logit}(0.65)$. This produces a response rate close to 65\%
and makes response systematically related to a size like covariate.
Because $r_i$ depends only on $\mathbf{x}_i$ and $\mathbf{x}_i$ enters $y_i$ through $g(\mathbf{w}_i)$,
respondent only estimators are biased, and estimators that do not account for $\mathbf{x}_i$ are not
well protected under this mechanism.

\subsection{Pretrained historical model}
\label{subsec:sim_pretrain}

For each benchmark we generate an external historical dataset of size $N_{\mathrm{hist}}=100{,}000$
using the historical covariate distribution described in Section~\ref{subsec:sim_covariates}. We fit
a prediction function $f_{\text{hist}}(\mathbf{z})$ using XGBoost regression with a 70/30
train validation split and hyperparameter tuning via \texttt{Optuna} (10 trials). The fitted
function is then treated as fixed and used to produce historical predictions for all units in the
population,
\[
\hat y_i^{\text{hist}} = f_{\text{hist}}(\mathbf{z}_i),
\qquad i\in U.
\]

\subsection{Estimators computed}
\label{subsec:sim_estimators}

We report results for two experiment types.

\paragraph{Complete response experiments.}
We set $r_i=1$ for all sampled units and compare the complete response estimators HT
(Equation~\eqref{eq:baseline_ht}), GREG, PPD, GREG$_{\hat y}$ (Equation~\eqref{eq:s2_greg_yhat_complete}),
and model calibrated estimation, as defined in Section~\ref{subsubsec:sim_estimators}.

\paragraph{Item nonresponse experiments.}
We generate $r_i$ as in Section~\ref{subsec:sim_nonresponse} and compare:
the naive respondent only estimator, the nonresponse weighting estimator NWA
(Equation~\eqref{eq:s1_nwa}), the standard NR GREG estimator using only frame variables
(Equation~\eqref{eq:s2_greg}), the two stage mass imputation projection estimator
(Equation~\eqref{eq:s3_mass_imp}), Hist PPD (Equation~\eqref{eq:s4_hist_ppd}), and the proposed
Hist PPD DR estimator (Equation~\eqref{eq:s5_hist_ppd_dr}).

\subsection{Variance estimation and confidence intervals}
\label{subsec:sim_variance}

For each estimator and each Monte Carlo replicate we estimate variance using the design based
bootstrap described in Section~\ref{sec:variance_estimation}. In each bootstrap replicate we refit
any models that would be re estimated in practice. The historical function $f_{\text{hist}}$ and the
population level frame data $\{\mathbf{z}_i : i\in U\}$ are treated as fixed.

We form nominal 95\% confidence intervals of the form
\[
\left[\hat\mu - 1.96\sqrt{\widehat{\mathrm{Var}}(\hat\mu)},\ \hat\mu + 1.96\sqrt{\widehat{\mathrm{Var}}(\hat\mu)}\right].
\]

\subsection{Performance metrics}
\label{subsec:sim_metrics}

Let $\hat\mu_k$ denote the estimate in Monte Carlo replicate $k=1,\dots,K$ and let $\widehat{V}_k$
denote its bootstrap variance estimate. We report:

\paragraph{Relative bias.}
\[
\mathrm{RB}(\hat\mu)
=
100 \times \frac{1}{K}\sum_{k=1}^K \frac{\hat\mu_k - \mu}{\mu}.
\]

\paragraph{Relative root mean squared error.}
\[
\mathrm{RRMSE}(\hat\mu)
=
100 \times
\sqrt{\frac{1}{K}\sum_{k=1}^K \left(\frac{\hat\mu_k - \mu}{\mu}\right)^2 }.
\]
...
\mathbbm{1}\left\{
\mu \in \left[\hat\mu_k - 1.96\sqrt{\widehat{V}_k},\ \hat\mu_k + 1.96\sqrt{\widehat{V}_k}\right]
\right\}.
\]

\paragraph{Mean confidence interval width.}
\[
\mathrm{CIW}(\hat\mu)
=
\frac{1}{K}\sum_{k=1}^K 2\times 1.96\sqrt{\widehat{V}_k}.
\]
